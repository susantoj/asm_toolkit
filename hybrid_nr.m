% HYBRID_NR -  Hybrid genetic algorithm and NR solver to optimise Rs and Xr2
%              Uses NR_SOLVER2 as the base newton-raphson solver
%			   Note that NR_SOLVER2 is simply the conventional NR_SOLVER 
%			   with Rs and Xr2 as known inputs
%              Optimises Rs and Xr2 with genetic algorithm
%

function [z err conv] = hybrid_nr(p, max_iter)
...
gen = 1
for i=1:pop    
    [x(i,:) iter(i) err(i) conv(i)] = nr_solver2(p, Rs(i,1), Rs(i,2), max_iter);
    if conv(i) == 1
        z = x(i,:);
        conv = 1;
        err = err(i);
