% HYBRID_DNR - Hybrid genetic algorithm and LM solver to optimise Rs and Xr2
%              Uses DNR_SOLVER2 as the base newton-raphson solver
%			   Note that DNR_SOLVER2 is simply the conventional DNR_SOLVER 
%			   with Rs and Xr2 as known inputs
%              Optimises Rs and Xr2 with genetic algorithm
%

function [z err conv] = hybrid_dnr(p, max_iter)
...
gen = 1
for i=1:pop    
    [x(i,:) iter(i) err(i) conv(i)] = dnr_solver2(p, Rs(i,1), Rs(i,2), 1e-7, max_iter);
    if conv(i) == 1
        z = x(i,:);
        conv = 1;
        err = err(i);
