% LM_SOLVER2 - Levenberg-Marquadt solver for double cage model with core losses
...
%              Gain ratio adjustment of damping parameter
%
% Usage: lm_solver2 (p, kr, kx, lambda_0, lambda_max, max_iter)
%
...
%		  kr and kx are linear restrictions
...
function [z iter err conv] = lm_solver2(p, kr, kx, lambda_0, lambda_max, max_iter)
...
while (err > err_tol) && (iter < max_iter)
    ...
    while (iter == iter0)
        ...
        dE = err0 - err;
        dL = -delta_x'*(-lambda*delta_x - j'*y')/2;
        rho = dE/dL;
        ...
        % Gain ratio adjustment of lambda
        % *******************************
        if (rho > 0)
            iter = iter + 1;
        else
